...
export class PublicServices {
  ...
  async fetchMessages(chat_id) {
    ...
    // (public_url / order_index). Prefer the durable storage public URL when the
    // stored url is a transient data: URL from image generation.
    return (data ?? []).map((msg) => ({
      ...msg,
      imagesData: (msg.imagesData ?? []).map((img) => {
        let publicUrl = img.url ?? "";

        if (img.storage_path && (!publicUrl || publicUrl.startsWith("data:"))) {
